...
# uncomment to use the module six
# import six
import csv
import datetime
import logging
...
def users(request, gym_pk):
    ...
    # Should this requirement be dropped once, they can be removed.
    writer.writerow([
        _('Nr.'), _('Gym').encode('utf8'), _('Username').encode('utf8'),
        _('Email').encode('utf8'), _('First name').encode('utf8'),
        _('Last name').encode('utf8'), _('Gender').encode('utf8'),
        _('Age').encode('utf8'), _('ZIP code').encode('utf8'),
        _('City').encode('utf8'), _('Street').encode('utf8'),
        _('Phone').encode('utf8')
    ])
    for user in Gym.objects.get_members(gym_pk):
        address = user.userprofile.address
        writer.writerow([
            user.id, gym.name.encode('utf8'), user.username, user.email,
