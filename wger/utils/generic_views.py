...
class WgerMultiplePermissionRequiredMixin(PermissionRequiredMixin):
    '''
    A PermissionRequiredMixin that checks that the
...
class WgerPermissionMixin(object):
    ...
    This can be a string or a tuple,
    in the latter case having any of the permissions
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.login_required or self.permission_required:
            if not request.user.is_authenticated():
                return HttpResponseRedirect(
                    reverse_lazy('core:user:login') + '?next={0}'.format(
                        request.path))
        ...
        # Dispatch normally
        return super(WgerPermissionMixin, self).dispatch(request, *args, **
                                                         kwargs)
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.form_action_urlname:
            context['form_action'] = reverse(
                self.form_action_urlname, kwargs={'pk': self.object.id})
