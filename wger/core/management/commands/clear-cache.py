...
# uncomment to use the module below
...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (make_option(
        ...
        dest='clear_template',
        default=False,
        help='Clear only template caches'), make_option(
            '--clear-workout-cache',
            action='store_true',
            dest='clear_workout',
            default=False,
            help='Clear only the workout canonical view'), make_option(
                '--clear-all',
                action='store_true',
                dest='clear_all',
                default=False,
    ...
    def handle(self, **options):
        ...
        if (not options['clear_template'] and not options['clear_workout'] and
                not options['clear_all']):
            raise CommandError(
            ...
            for user in User.objects.all():
                ...
                for entry in WorkoutLog.objects.filter(user=user).dates(
                    ...
                    for month in WorkoutLog.objects.filter(
                            user=user, date__year=entry.year).dates('date',
                                                                    'month'):
                        if int(options['verbosity']) >= 3:
                            self.stdout.write(
