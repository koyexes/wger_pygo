...
class Workout(models.Model):
    ...
    comment = models.CharField(verbose_name=_('Description'),
                               ...
                               blank=True,
                               help_text=_(
                                   "A short description or goal of the "
                                   + " workout. For example 'Focus on back'"
                                   + " or 'Week 1 of program xy'."))
    ...
    def canonical_representation(self):
        ...
        or part of a workout structure is needed. As an additional benefit,
...
class ScheduleManager(models.Manager):
    ...
    def get_current_workout(self, user):
        '''
        Finds the currently active workout for the user, by
        checking the schedules and the workouts
        :rtype : list
        '''
        ...
        try:
            ...
            if schedule.schedulestep_set.count():
                # The schedule might exist and have
                # steps, but if it's too far in
                ...
                active_workout = schedule\
                    .get_current_scheduled_workout().workout
            else:
                # same as above
                raise ObjectDoesNotExist
        ...
        # there are no active schedules, just return the last workout
            ...
            schedule = False
            try:
                active_workout = Workout.objects.filter(user=user)\
...
class WorkoutLog(models.Model):
    ...
    Note that since adding the unit field,
    the name is no longer correct, but is
    kept for compatibility reasons (specially for the REST API).
    '''
...
class WorkoutSession(models.Model):
    ...
    def clean(self):
        ...
        if (not self.time_end and self.time_start) \
                or (self.time_end and not self.time_start):
            raise ValidationError(_(
                "If you enter a time, you must enter both"
                + " start and end time."))
