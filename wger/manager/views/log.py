...
class WorkoutLogUpdateView(WgerFormMixin, UpdateView, LoginRequiredMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['title'] = _(u'Edit log entry for %s') % self\
...
def add(request, pk):
    ...
    # We need several lists here because we need to
    # assign specific form to each
    # exercise: the entries for weight and repetitions have
    # no indicator to which
    ...
    if request.method == 'POST':
        ...
        for form_id in form_to_exercise:
            if post_copy.get('form-%s-weight' % form_id) \
                    or post_copy.get('form-%s-reps' % form_id):
        ...
        if dateform.is_valid() and session_form.is_valid() \
            ...
            return HttpResponseRedirect(reverse(
                'manager:log:log', kwargs={'pk': day.training_id}))
    else:
        ...
        # update the current one or create a new one
...
class WorkoutLogDetailView(DetailView, LoginRequiredMixin):
        ...
        for day_list in self.object.canonical_representation['day_list']:
            ...
            for set_list in day_list['set_list']:
                ...
                for exercise_list in set_list['exercise_list']:
                    ...
                    # Filter the logs for user and exclude all units
                    # that are not weight
                    #
                    # TODO: add the repetition_unit to the filter.
                    #       For some reason (bug  in django? DB problems?)
                    #       when adding the filter there, the
                    #       execution time explodes.
                    #       The weight unit filter works as
                    #       expected. Also, adding the unit IDs
                    #       to the exclude list  also has the disadvantage
                    #       that if new ones are added in a
                    #       local instance, they could "slip" through.
                    logs = exercise_list['obj'].workoutlog_set\
                        .filter(user=self.owner_user,
                                weight_unit__in=(1, 2),
                                workout=self.object) \
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        # Dispatch normally
        return super(WorkoutLogDetailView, self).dispatch(request, *args,
                                                          **kwargs)
