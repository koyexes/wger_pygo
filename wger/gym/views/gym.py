...
# flake8 flagged unused import error but some methods here are dependent on
# the module
from wger.gym.helpers import (get_user_last_activity,
                              is_any_gym_admin, get_permission_list)
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if request.user.has_perm('gym.manage_gyms') \
            or ((request.user.has_perm('gym.manage_gym')
                ...
                and request.user.userprofile.gym_id ==
                int(self.kwargs['pk'])):
...
def gym_new_user_info_export(request):
    ...
    writer.writerow([
        _('Username'), _('First name'), _('Last name'), _('Gym'), _('Password')
    ])
    writer.writerow([
