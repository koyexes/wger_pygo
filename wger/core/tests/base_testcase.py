...
class WorkoutManagerDeleteTestCase(WorkoutManagerTestCase):
    '''
    Tests deleting an object an authorized user, a different
    one and a logged out one. This assumes the delete action
    is only triggered with a POST request and
...
class WorkoutManagerEditTestCase(WorkoutManagerTestCase):
    ...
    a different one and a logged out one.
    '''
    ...
    fileupload = None
    '''
    If the form requires a file upload, specify the
    field name and the file path here in a list or tuple:
    ...
    def edit_object(self, fail=False):
        ...
        if self.fileupload:
            ...
            response = self.client.post(
                get_reverse(self.url, kwargs={'pk': self.pk}), self.data)
        ...
        if fail:
            ...
            # TODO: the redirection page might
            #  not have a language prefix (e.g. /user/login
            #  instead of /en/user/login) so there is an additional redirect
            # # The page we are redirected to doesn't trigger an error
            # response = self.client.get(response['Location'])
            # self.assertEqual(response.status_code, 200)
...
class WorkoutManagerAddTestCase(WorkoutManagerTestCase):
    ...
    fileupload = None
    '''
    If the form requires a file upload,
    specify the field name and the file path
    ...
    def add_object(self, fail=False):
        ...
        if fail:
            ...
            # TODO: the redirection page might not
            # have a language prefix (e.g. /user/login
            # instead of /en/user/login) so there is an additional redirect
            # # The page we are redirected to doesn't trigger an error
            # response = self.client.get(response['Location'])
            # self.assertEqual(response.status_code, 200)
