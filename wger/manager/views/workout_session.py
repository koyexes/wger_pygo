...
class WorkoutSessionUpdateView(WgerFormMixin, LoginRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(WorkoutSessionUpdateView, self)\
            .get_context_data(**kwargs)
        context['form_action'] = reverse('manager:session:edit',
                                         kwargs={'pk': self.object.id})
        context['title'] = _('Edit workout impression for {0}')\
            .format(self.object.date)
...
class WorkoutSessionAddView(WgerFormMixin, LoginRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        return super(WorkoutSessionAddView, self).dispatch(request, *args,
                                                           **kwargs)
...
class WorkoutSessionDeleteView(WgerDeleteMixin, LoginRequiredMixin,
    ...
    def delete(self, request, *args, **kwargs):
        '''
        Delete the workout session and, if wished, all associated
        weight logs as well
        '''
        ...
        return super(WorkoutSessionDeleteView, self).delete(request, *args,
                                                            **kwargs)

    def get_context_data(self, **kwargs):

        logs = '' if not self.kwargs['logs'] else self.kwargs['logs']
        context = super(WorkoutSessionDeleteView, self)\
            .get_context_data(**kwargs)
        context['form_action'] = reverse('manager:session:delete',
                                         kwargs={'pk': self.object.id,
                                                 'logs': logs})
