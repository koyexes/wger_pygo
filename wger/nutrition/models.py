    ...
    def get_nutritional_values(self):
        ...
        if energy:
            for key in result['percent'].keys():
                result['percent'][key] = \
                    result['total'][key] * ENERGY_FACTOR[key][unit] \
                    / energy * 100
...
class Ingredient(AbstractLicenseModel, models.Model):
        ...
        if isinstance(other, self.__class__):
            for i in self._meta.fields:
                if (hasattr(self, i.name) and hasattr(other, i.name) and
                    (getattr(self, i.name, None) != getattr(other, i.name,
                                                            None))):
                    equal = False
        else:
            equal = False
    ...
    def send_email(self, request):
        '''
        Sends an email after being successfully
        added to the database (for user
...
class MealItem(models.Model):
    ...
    def get_nutritional_values(self, use_metric=True):
        ...
        nutritional_info[
            'carbohydrates'] += self.ingredient.carbohydrates \
            * item_weight / 100
        ...
        if self.ingredient.fat_saturated:
            nutritional_info[
                'fat_saturated'] += self.ingredient.fat_saturated \
                * item_weight / 100

        if self.ingredient.fibres:
            nutritional_info[
                'fibres'] += self.ingredient.fibres * item_weight / 100

        if self.ingredient.sodium:
            nutritional_info[
                'sodium'] += self.ingredient.sodium * item_weight / 100
